...
def Model(X_train, X_test, y_train, y_test):
    input = layer.Input(shape=[shape, shape, 1])
    ...
    pool = layer.MaxPool2D(pool_size=k_s_list[0][0], strides=k_s_list[0][1])(conv)

    # FPN
    conv1 = layer.Conv2D(filters=64, kernel_size=3, strides=1, padding="valid", activation=ACTIVATION,
                         kernel_initializer=tf.keras.initializers.he_uniform())(pool)
